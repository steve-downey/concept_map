...
inline constexpr struct partial_eq_default_t {
    constexpr bool eq(const has_eq auto& rhs, const has_eq auto& lhs) const { return (rhs == lhs); }
    constexpr bool ne(const has_eq auto& rhs, const has_eq auto& lhs) const { return (lhs != rhs); }
} partial_eq_default;
